...
class DateExpression extends Expression {
    ...
    public static function parse($text) {
        ...
        $expressions = array(
            '/(?P<date>\d{2}\sde\s[a-z]{5,10}\sde\s\d{4})/' => 'datecomplete',
            '/(?P<date>\d{2}-\d{2}\sde\s[a-z]{5,10}\sde\s\d{4})/' => 'periodday',
            '/(?P<date>\d{2}\sde\s[a-z]{5,10}-\d{2}\sde\s[a-z]{5,10}\sde\s\d{4})/' => 'periodmonth',
            '/(?P<date>\d{4}\/\d{4})/' => 'periodyear',
            '/(?P<date>\d{4})/' => 'year',
        );
