...
foreach ($referencias as $index => $referencia) {
    ...
    $matchComplete = ($matchAuthor && $matchDate  && $matchTtile) ? true : false; 
    $matchIncomplete = ($matchAuthor || $matchDate || $matchTitle) ? true : false; 
    
    fputcsv($handleCsv, [$referencia, $matchAuthor, $matchDate, $matchTtile, $matchUrl, $matchComplete, $matchIncomplete, "", $matchType]);
...
if (($handleCsv = fopen($fileCsv, 'r')) !== false) {
    ...
    while (($data = fgetcsv($handleCsv)) !== false) {
        ...
        if (isset($data[5]) && $data[5] == 1) {
            $matchCompleteCount++;
            $matchCompleteRows[] = $totalRows; // Almacena el número de fila (totalRows) con `matchComplete = 1`
        }
    }
    fclose($handleCsv);
}
...
echo "\nFilas con matchComplete = 1: " . implode(", ", $matchCompleteRows) . "\n";
?>
